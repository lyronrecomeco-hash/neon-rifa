...
import { Shuffle, Plus, Trash2, ShoppingCart, X } from 'lucide-react';
...
export function SelectionPanel({
  ...
  const handleAddManual = () => {
    ...
    if (success) {
      ...
      setError('Este número já foi comprado ou selecionado');
    }
  };
  ...
  return (
    <div className="glass rounded-2xl p-6 space-y-6 shadow-card">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <label className="text-sm text-muted-foreground">Adicionar número</label>
        ...
        {/* Random selection */}
        <div className="space-y-2">
          <label className="text-sm text-muted-foreground">Seleção aleatória</label>
          <div className="flex gap-2">
            <Input
              type="number"
              placeholder="Qtd"
      ...
      {selectedCount > 0 && (
        <div className="space-y-3 animate-fade-in">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <span>{selectedCount} número{selectedCount > 1 ? 's' : ''} selecionado{selectedCount > 1 ? 's' : ''}</span>
          </div>
          <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto custom-scrollbar">
            {sortedNumbers.map((num) => (
              <button
                ...
                className="group flex items-center gap-1 px-3 py-1.5 rounded-full bg-primary/20 border border-primary/40 text-sm font-medium hover:bg-destructive/20 hover:border-destructive/40 transition-colors"
              >
      ...
      <div className="pt-4 border-t border-border/50 space-y-4">
        ...
        >
          ...
          {selectedCount === 0 ? 'Selecione números' : 'Comprar números'}
        </Button>
      </div>
    </div>
  );
